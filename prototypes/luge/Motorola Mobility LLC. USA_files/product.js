...
//				}				
			...
			var getAddToCartBtn = function(thisProduct) {
				var addToCartBtn = jQuery(thisProduct.containerId+" .addtocartbutton:last").click(function(e) {
					...
					if (model.bundle) {
						...
						thisProduct.selectedOptions.Quantity = jQuery(thisProduct.containerId+" .quantityselect option:selected").val();
					}
					...
					if (model.productSet || thisProduct.selectedOptions.Quantity > 0) {
						...
						var quantity=jQuery(thisProduct.containerId+" .quantityselect option:selected").val();
						...
						if(thisProduct.master){
							if(thisProduct.selectedVar.id!=""){
								url = app.util.appendParamToURL(app.URLs.updateVariationInventory,"pid", thisProduct.selectedVar.id);
								...
								jQuery.ajax({
									...
									success	: function(data) {
										webtrendsAddToCart(data.WT_ProductName,data.WT_ProductCategory,data.WT_SubCategory,data.WT_SKUName,quantity,data.WT_ID);
										var exceededQuota=data.exceededQuota;
										if(exceededQuota==1){
											jQuery(thisProduct.containerId+" .pricing span.qty-error").remove();	
											if(data.unavailable==1){
												jQuery(thisProduct.containerId+" .pricing").append("<span class=\"qty-error\">"+app.resources["OUTOFSTOCK"]+"</span>");
											}else if(data.unavailable==0){
												jQuery(thisProduct.containerId+" .pricing").append("<span class=\"qty-error\">"+app.resources["LIMIT_EXCEEDED"]+"</span>");
											}
											
											var maxQty=data.maxQty;
											jQuery(thisProduct.containerId+" .quantityselect").html("");
											var option="";
											for(i=1;i<=maxQty;i++){
												option+="<option value=\""+i+"\">"+i+"</option>";
											}
											jQuery(thisProduct.containerId+" .quantityselect").append(option);
										}else if(exceededQuota==0){
											app.minicart.add("${URLUtils.staticURL('/images/loading-progress-98x22.gif')}", {"pid": thisProduct.selectedVar.id,"Quantity":quantity}); 
											jQuery(thisProduct.containerId+" .pricing span.qty-error").remove();
											jQuery.ajax({
												type	: "GET",
												url		: url,
												...
												success	: function(data) {
													...
													if(jQuery(thisProduct.containerId+" .quickViewProdRec").length>0){
														var hasQuickViewRec=jQuery(thisProduct.containerId+" .quickViewProdRec").val();
														
														if(hasQuickViewRec*1>0){
															var RecUrl = app.URLs.showRecommendations+"?pid="+thisProduct.pid;
															window.setTimeout(function () { app.RecommendationsView.show({url: RecUrl}); }, 500);
														}
													}
												}
											});
							...
							if(thisProduct.pid!=""){
								url = app.util.appendParamToURL(app.URLs.updateVariationInventory,"pid", thisProduct.pid);
								...
								jQuery.ajax({
									...
									success	: function(data) {
										...
										if(exceededQuota==1){
											...
											app.minicart.add("${URLUtils.staticURL('/images/loading-progress-98x22.gif')}", {"pid": thisProduct.pid,"Quantity":quantity});  
											jQuery(thisProduct.containerId+" .pricing span.qty-error").remove();
			...
			// binds A2C button click handler
			var getAddToCartVariantBtn = function(thisProduct) {
				var addToCartVariantBtn = jQuery(thisProduct.containerId+" .addtocartbtn:last").click(function(e) {
					var productId = jQuery(thisProduct.containerId+" .selectProductRadio:checked").val();
					var quantity=jQuery(thisProduct.containerId+" .quantityselect option:selected").val();
					
					var WTData=jQuery(thisProduct.containerId+" .selectProductRadio:checked").data("data");
															
					if(productId!=""){
						if(jQuery(thisProduct.containerId+" .selectProductRadio:checked").parent().parent().hasClass('hasPlan')){
							var urls=jQuery(thisProduct.containerId+" .selectProductRadio:checked").next("[name='buyPlanLink']").val();
							urls=urls.split("|^|");
							webtrendsBuyWithPlan(WTData.WTName,WTData.WTCat,WTData.WTSub,WTData.WTSku,quantity,WTData.WTId);
						
							//browser detection: http://msdn.microsoft.com/en-us/library/ms537509(v=vs.85).aspx
							if(navigator.appName == 'Microsoft Internet Explorer') {
								//include the referrer
								var simplexityLink = jQuery("#simplexityLink");
								simplexityLink.attr("href", urls[quantity-1]);
								jQuery("#simplexityLink")[0].click();
							}
							else {
								window.open(urls[quantity-1]);
							}
						}else{
							webtrendsAddToCart(WTData.WTName,WTData.WTCat,WTData.WTSub,WTData.WTSku,quantity,WTData.WTId);
							app.minicart.add("${URLUtils.staticURL('/images/loading-progress-98x22.gif')}", {"pid": productId,"Quantity":quantity});  
							
							var hasRec=jQuery(thisProduct.containerId+" .hasRecommendation").val();
							
							app.BuyItView.close();
							
							if(hasRec*1>0){
								var RecUrl = app.URLs.showRecommendations+"?pid="+productId;
								window.setTimeout(function () { app.RecommendationsView.show({url: RecUrl}); }, 500);
							}
						}
				...
				return addToCartVariantBtn;
			}
			
			// binds A2C button click handler
			var getAddToCartRecBtn = function(thisProduct) {
				var addToCartRecBtn = jQuery(thisProduct.containerId+" .addToCartRecommendation:last").click(function(e) {
					e.preventDefault();
					var quantity=jQuery("#RecommendationsDialog "+thisProduct.containerId+" .quantityselect option:selected").val();
					if(thisProduct.master){
						if(thisProduct.selectedVar.id!=""){
							url = app.util.appendParamToURL(app.URLs.updateVariationInventory,"pid", thisProduct.selectedVar.id);
							...
							jQuery.ajax({
								...
								webtrendsAddToCart(data.WT_ProductName,data.WT_ProductCategory,data.WT_SubCategory,data.WT_SKUName,quantity,data.WT_ID);
									var exceededQuota=data.exceededQuota;
									if(exceededQuota==1){
										jQuery(thisProduct.containerId+" .rec-pricing span.qty-error").remove();	
										if(data.unavailable==1){
											jQuery(thisProduct.containerId+" .rec-pricing").append("<span class=\"qty-error\">"+app.resources["OUTOFSTOCK"]+"</span>");
										}else if(data.unavailable==0){
											jQuery(thisProduct.containerId+" .rec-pricing").append("<span class=\"qty-error\">"+app.resources["LIMIT_EXCEEDED"]+"</span>");
										}
										...
										app.minicart.add("${URLUtils.staticURL('/images/loading-progress-98x22.gif')}", {"pid": thisProduct.selectedVar.id,"Quantity":quantity}, function(){
											
											if(jQuery(".recommendations-dialog:visible").length>0){
												jQuery.ajax({
													type	: "GET",
													url		: app.URLs.updateyoujustadded,
													cache	: true,
													dataType : 'html',
													success	: function(data) {
														jQuery('#RecSummary').empty()
															.html(data);
													}
												});
											}
							...
							jQuery.ajax({
								...
								success	: function(data) {
									...
									if(exceededQuota==1){
										...
										app.minicart.add("${URLUtils.staticURL('/images/loading-progress-98x22.gif')}", {"pid": thisProduct.pid,"Quantity":quantity}, function(){
				...
				return addToCartRecBtn;
			}
			
		
			
			// binds A2C button click handler
			var getAddToCartFromCartRecBtn = function(thisProduct) {
				var addToCartFromCartRecBtn = jQuery(thisProduct.containerId+" .addToCartRec:last").click(function(e) {
					e.preventDefault();
					
					function input_blur(container){
						var current_qty=Number($(container).val());
						var max_val=Number($(container).next('.maxQtyRec').val());
						if(max_val>0){
							if(current_qty>max_val){
								var error_msg='<span class="error">'+app.resources["MAXIMUM_QUANTITY"]+'</span>';
								if($(container).parent().children('.error').length==0){
									$(container).parent().append(error_msg);
								}else{
					...
					var cur_this=$(this).parent().prevAll(".rec-pricing").children(".qty-val");
					input_blur(cur_this);

					if($(cur_this).parent().children('.error').length==0 && !$(cur_this).hasClass("recerrorclient")){
						var currentContainer=$(this);	
						if(thisProduct.selectedVar.id!=""){
							var postdata = "qty="+$(this).parent().prevAll(".rec-pricing").children(".qty-val").val()+"&pid="+thisProduct.selectedVar.id+"&checkIfProdInCart=true";	
						}else{
							var postdata = "qty="+$(this).parent().prevAll(".rec-pricing").children(".qty-val").val()+"&pid="+$(this).next(".prodID").val()+"&checkIfProdInCart=true";	
						}
						jQuery.ajax({
							type	: "POST",
							url		: app.URLs.addToCartUrl,
							cache	: false,
							data	: postdata,
							dataType: "json",
							success	: function(data){
								if(data.error==null){
									...
									var error_msg='<span class="error">'+data.error+'</span>';
									if($(currentContainer).parent().prevAll(".rec-pricing").children(".error").length>0){
										$(currentContainer).parent().prevAll(".rec-pricing").children(".error").remove();
									}
									$(currentContainer).parent().prevAll(".rec-pricing").append(error_msg);	
								}
							},
							error	: function(){
				...
				return addToCartFromCartRecBtn;
			}
			
			var getAddToCartAccBtn = function(thisProduct) {
				var addToCartAccBtn = jQuery(thisProduct.containerId+" .addToCartAcc:last").click(function(e) {
					e.preventDefault();
					var quantity=jQuery(thisProduct.containerId+" .quantityselect option:selected").val();
					if(thisProduct.master || thisProduct.variant){
						if(thisProduct.selectedVar.id!=""){
							...
							jQuery.ajax({
								...
								contentType : "application/json",
								success	: function(data) {
									webtrendsAddAccessory(data.WT_ProductName,data.WT_ProductCategory,data.WT_SubCategory,data.WT_SKUName,quantity,data.WT_ID);
									var exceededQuota=data.exceededQuota;
									if(exceededQuota==1){
										jQuery(thisProduct.containerId+" .rec-pricing span.qty-error").remove();	
										if(data.unavailable==1){
										...
										var hasRec=jQuery(thisProduct.containerId+" .prodAccRec").val();
										
										if(hasRec*1>0){
											var RecUrl = app.URLs.showRecommendations+"?pid="+thisProduct.pid;
											app.RecommendationsView.show({url: RecUrl});
										}
				...
				return addToCartAccBtn;
			}

			
			
			var updateVariantsQuantity = function(thisProduct) {
				jQuery(thisProduct.containerId+" .selectProductRadio").click(function () {
			        this.blur();
			        this.focus();
			   });
				var updateVariantsQuantity = jQuery(thisProduct.containerId+" .selectProductRadio").change(function(e) {
					
					var locale = jQuery(thisProduct.containerId+" .addtocartbtn").attr("locale");
					var productName = jQuery(thisProduct.containerId+" .addtocartbtn").attr("productname");
				    
					if(jQuery(this).parent().parent().hasClass('hasPlan')){
						jQuery(thisProduct.containerId+" .addtocartbtn").html("<span><span class=\"add-to-cart-basket\"></span>"+app.resources["BUY_PLAN"]+"</span>"); 
						jQuery(thisProduct.containerId+" .addtocartbtn").attr("title",app.resources["BUY_PLAN"]);
					}else{
						jQuery(thisProduct.containerId+" .addtocartbtn").html("<span><span class=\"add-to-cart-basket\"></span>"+app.resources["ADD_TO_CART"]+"</span>"); 
						jQuery(thisProduct.containerId+" .addtocartbtn").attr("title",app.resources["ADD_TO_CART"]);
					}
					
					var prom=".variantPromotion_"+jQuery(this).val();
					jQuery(thisProduct.containerId+" .modal-footer .variantPromos .variantPromo").hide();
					
					if(jQuery(thisProduct.containerId+" .modal-footer .variantPromos "+prom).length>0){
						jQuery(thisProduct.containerId+" .modal-footer .variantPromos "+prom).show();	
					}
					
					url = app.util.appendParamToURL(app.URLs.updateVariationInventory,"pid", jQuery(this).val());
					if(jQuery(this).val()!=""){
						jQuery.ajax({
							type	: "GET",
							url		: url,
							...
							success	: function(data) {
								...
								jQuery(thisProduct.containerId+" .quantityselect").append(option);
								jQuery(thisProduct.containerId+" .hasRecommendation").val(hasRec);
								
							}
						});
					}else{
						jQuery(thisProduct.containerId+" .quantityselect").html("");	
						jQuery(thisProduct.containerId+" .quantityselect").append("<option value=\"1\">1</option>");
						jQuery(thisProduct.containerId+" .hasRecommendation").val(0);
					}
					return false;
				});

				return updateVariantsQuantity;
			}
			...
			return {
				...
				bonusProduct		: false,
				customWebtrends     : model.customWebtrends,
				...
				showImages: function(selectedVal, vals)  {
					...
					jQuery.each(vals, function(){
						...
						if (this.val === selectedVal && this.images) {
							if (this.images.quickview.length > 0) {
								//jQuery(that.containerId+" .productthumbnailsquick:last").html("");
								jQuery(that.containerId+" .productimagequick").html("").append(jQuery("<img/>").attr("src", thisVal.images.quickview[0].url).attr("alt", thisVal.images.quickview[0].alt).attr("title", htmlEncode(thisVal.images.quickview[0].title)));
							}							
							
							if (this.images.medium.length > 0) {
								//jQuery(that.containerId+" .productthumbnailsquick:last").html("");
								if(jQuery(that.containerId).hasClass('access-prod')){
									jQuery(that.containerId+" img").remove();
									jQuery(that.containerId+" .image").append(jQuery("<img/>").attr("src", thisVal.images.medium[0].url).attr("alt", thisVal.images.medium[0].alt).attr("title", thisVal.images.medium[0].title));
								}
								if(jQuery(that.containerId).hasClass('cart-recommendation')){
									jQuery(that.containerId+" .img").empty();
									jQuery(that.containerId+" .img").append(jQuery("<img/>").attr("src", thisVal.images.medium[0].url).attr("alt", thisVal.images.medium[0].alt).attr("title", thisVal.images.medium[0].title));
								}
							}							
					
							// make sure to show number of images based on the smallest of large or small as these have to have 1-1 correspondence.
							var noOfImages = this.images.quickview.length;
							
							// show only 3 images
							if(noOfImages > 3) noOfImages = 3;
							
							jQuery(that.containerId+" .productthumbnailsquick").empty();
							
							// show thumbnails only if more than 1 or if this is a subproduct (bundled/subproduct)
							if ((this.images.quickview.length > 1  || that.isSubProduct()) && !that.isBonusProduct()) {
								jQuery.each(this.images.quickview, function(){
									imgCounter++;
									...
									if(imgCounter == 0)
										jQuery(that.containerId+" .productthumbnailsquick:last").append(jQuery("<img/>").attr("src", this.url).attr("alt", this.alt).attr("title", htmlEncode(this.title)).attr("class", "selected").click(function(e){
											jQuery(that.containerId+" .productimagequick").html("").append(jQuery("<img/>").attr("src", thisVal.images.quickview[imageInd].url).attr("alt", thisVal.images.quickview[imageInd].alt).attr("title", htmlEncode(thisVal.images.quickview[imageInd].title)));
											jQuery('.productthumbnailsquick img').removeClass("selected");
											  jQuery(this).addClass("selected");
										}));
									else
										jQuery(that.containerId+" .productthumbnailsquick:last").append(jQuery("<img/>").attr("src", this.url).attr("alt", this.alt).attr("title", htmlEncode(this.title)).click(function(e){
											jQuery(that.containerId+" .productimagequick").html("").append(jQuery("<img/>").attr("src", thisVal.images.quickview[imageInd].url).attr("alt", thisVal.images.quickview[imageInd].alt).attr("title", htmlEncode(thisVal.images.quickview[imageInd].title)));
											jQuery('.productthumbnailsquick img').removeClass("selected");
											  jQuery(this).addClass("selected");
										}));
				...
				* */
				...
				refreshView: function() {
					...
					if (!isLoadingVar && this.selectedVar != null) {
						//update sku on quickview
						if(jQuery('.itemNo.productid').length>0){
							jQuery('.itemNo.productid .prosku').html(this.selectedVar.id);
						}
						
						// change description text on quickview
						if(jQuery(".variants-desc").length>0){
							jQuery(".variants-desc").hide();	
							jQuery(".variants-desc ."+this.selectedVar.id).parent().show();
						}
						// update availability
						...
						// update price
						if(jQuery(".recommendations-dialog:visible").length>0){
							jQuery(thisProduct.containerId+" .image .img img").attr("src",this.selectedVar.mediumImage);
						}
						
						if(jQuery(".quickview:visible").length>0){
							var prom=".variantPromotion_"+thisProduct.selectedVar.id;
							jQuery(thisProduct.containerId+" .modal-footer .variantPromos .variantPromo").hide();
				...
				showUpdatedPrice: function(sale, standard) {
					...
					//show prices for accessories tab products
					if(jQuery(this.containerId+" .rec-pricing .price").length>0){
						jQuery(this.containerId+" .rec-pricing .price:first").html(priceHtml);
					}
				...
				*/
				...
				show: function(options) {
					...
					if(jQuery('.itemNo.productid').length>0){
						if(thisProduct.selectedVar!=null){
							jQuery('.itemNo.productid .prosku').html(thisProduct.selectedVar.id);
						}
					}
					...
					getAddToCartVariantBtn(this);
					
					getAddToCartRecBtn(this);
					
					getAddToCartAccBtn(this);
					
					getAddToCartFromCartRecBtn(this);
					
					updateVariantsQuantity(this);
